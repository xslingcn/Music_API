...
class Badge extends BaseController
{
    public function __call($room_id, $args)
    {
        ...
        $song = [
            'name' => '歌曲读取中',
            'singer' => 'Loading...',
            'pic' => 'https://api.x3l.zone/new/images/loading.png'
        ];
        $userName = '';
        if ($now) {
            $song = [
        ...
        $song["bg"] = "data:image/jpeg;base64," . base64_encode(file_get_contents("https://api.x3l.zone/new/images/player_bg.png"));

        $song["bar"] = "data:image/jpeg;base64," . base64_encode(file_get_contents("https://api.x3l.zone/new/images/player_bar.png"));
...
<filter id="filter_image" x="0" y="0">
  <feGaussianBlur stdDeviation="10" />
    ...
    <a xlink:href="https://music.x3l.zone/{$room['room_id']}" target="_blank" style="cursor:pointer;">
    ...
    <a xlink:href="https://music.x3l.zone/" target="_blank" style="cursor:pointer;">
        <g font-family="Consolas, PingFangSC-Regular, Microsoft YaHei" font-size="12" text-anchor="right">
            <text size="16" x="320" y="158" fill="#aaa" >music.x3l.zone</text>
        </g>
    </a>
...
XMLDATA;
    ...
    public function player()
    {
        ...
        $room = $roomModel->getRoomById($room_id);
        if (!$room) {
            header('Location: https://music.x3l.zone');
            return;
        }
